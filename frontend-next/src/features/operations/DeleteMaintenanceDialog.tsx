...
function Deletion(props:Props){
 ...
 <DialogFooter><Button variant="outline" disabled={running} onClick={close}>{started?'Close':cancelling?'Back':'Cancel'}</Button>{!started&&<Button variant="destructive" disabled={changed||(cancelling&&!reason.trim())} onClick={()=>void run()}>{cancelling?'Cancel selected windows':'Delete selected windows'}</Button>}</DialogFooter></DialogContent></Dialog>;
}
