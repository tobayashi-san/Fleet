...
function OsUpdatePreviewSession({serverId, environmentId, context}: {serverId:string; environmentId:string; context:string}) {
  ...
  return <div className="space-y-3 border-t px-4 py-3">
    <div className="flex flex-wrap items-center gap-3">
      <Button type="button" variant="outline" size="sm" disabled={preview.isPending} onClick={()=>preview.mutate({serverId,context})}>{preview.isPending ? 'Simulating updates…' : 'Preview package changes'}</Button>
      <p className="text-xs text-muted-foreground">Refreshes package metadata and simulates the update. No packages are installed or removed. The remote command is limited to 90 seconds.</p>
    </div>
    ...
    {result && <div className="space-y-2 text-xs">
      <p>Checked: {formatDateTime(result.checked_at)}</p>
      {result.plan ? <>
        <p className="text-muted-foreground">Full upgrade with automatic removal of unused packages. This is a point-in-time estimate; repository and host changes can alter the executed plan.</p>
        ...
        {result.plan.changes.length > 0 && <div className="space-y-2 rounded-md border p-3">
          ...
          <p className="text-muted-foreground">Based on service definitions owned by the installed packages. This does not establish which services are running or will restart. Shared libraries, package scripts and newly installed files may affect additional services.</p>
