...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'fleet-application-backup-'));const data=path.join(root,'data');fs.mkdirSync(data);fs.mkdirSync(path.join(root,'playbooks'));
...
const db=new Database(path.join(data,'fleet.db'));db.pragma('journal_mode = WAL');db.exec('CREATE TABLE users(id TEXT,token_version INTEGER); CREATE TABLE app_settings(key TEXT,value TEXT); CREATE TABLE environments(id TEXT);');
...
function malicious(name,entries){
 const chunks=[Buffer.from('FLEET-FILES-1\n')];
...
test('CLI verifies the encrypted package and database writes during creation abort publication',async()=>{
 ...
 const stdout=execFileSync(process.execPath,[path.join(__dirname,'../cli/application-backup.js'),'verify',path.join(root,'application.backup')],{encoding:'utf8',env:{...process.env,FLEET_BACKUP_PASSPHRASE:passphrase}});
...
test('application restore prepares database and files without overwriting original paths',async()=>{
 ...
 assert.equal(fs.existsSync(path.join(target,'files','data','fleet.db')),false);
 assert.equal(fs.existsSync(path.join(target,'files','data','fleet.db-wal')),false);
...
test('CLI application restore creates a reviewable recovery directory',()=>{
 ...
 const result=JSON.parse(execFileSync(process.execPath,[path.join(__dirname,'../cli/application-backup.js'),'restore',path.join(root,'application.backup'),target],{encoding:'utf8',env:{...process.env,FLEET_BACKUP_PASSPHRASE:passphrase}}));
 ...
 assert.equal(fs.readdirSync(root).some(name=>name.startsWith('.fleet-application-verify-')),false);
});
test('application backup resolves configured root and database aliases without copying raw SQLite files',async()=>{
 const alias=path.join(root,'data-alias');fs.symlinkSync(data,alias);const dbAlias=path.join(root,'db-alias.sqlite');fs.symlinkSync(path.join(data,'fleet.db'),dbAlias);
...
test('completed encrypted application package is authenticated before publication',async()=>{
 ...
 fsp.appendFile=async(filename,data,...args)=>{
  ...
  if(!changed && path.basename(filename)==='encrypted'){
   ...
   if(header.startsWith('FLEET-APPLICATION-1')){
...
test('prepared recovery is compared to the authenticated archive including database, files, plan and extra members',async()=>{
 ...
 assert.equal((await verifyPreparedApplicationRecovery(archive,target,passphrase)).preparedVerified,true);
 const result=JSON.parse(require('node:child_process').execFileSync(process.execPath,[path.join(__dirname,'../cli/application-backup.js'),'verify-prepared',archive,target],{encoding:'utf8',env:{...process.env,FLEET_BACKUP_PASSPHRASE:passphrase}}));
...
test('application key CLI authenticates encrypted archived core values without exposing secrets',async()=>{
 const {execFileSync}=require('node:child_process');
 const previous=process.env.FLEET_KEY_SECRET;process.env.FLEET_KEY_SECRET='application-recovery-test-key';
 ...
 try{
  ...
  const env={...process.env,FLEET_BACKUP_PASSPHRASE:passphrase};
  ...
  assert.ok(!output.includes('must-not-appear-in-output'));assert.ok(!output.includes(process.env.FLEET_KEY_SECRET));
  assert.throws(()=>execFileSync(process.execPath,args,{encoding:'utf8',env:{...env,FLEET_KEY_SECRET:'wrong-key'},stdio:'pipe'}),/does not authenticate/);
 }finally{if(previous===undefined)delete process.env.FLEET_KEY_SECRET;else process.env.FLEET_KEY_SECRET=previous;}
});
...
test('key verification includes managed SSH ciphertext and rejects mixed keys without revealing content',async()=>{
 ...
 try{
  ...
  const result=await verifyApplicationDatabaseKey(archive,passphrase,process.env.FLEET_KEY_SECRET);
  ...
  process.env.FLEET_KEY_SECRET='different-ssh-key';
