...
test('bundled update playbook previews then upgrades a real isolated Debian package', {timeout:240000}, async () => {
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'shipyard-package-integration-'));
  const name=`shipyard-review-update-${process.pid}`;
...
for version in 1.0 2.0; do
  mkdir -p /package/DEBIAN /package/usr/share/shipyard-review
  printf 'Package: shipyard-review-fixture\nVersion: %s\nArchitecture: all\nMaintainer: Test <test@example.invalid>\nDescription: Disposable integration fixture\n' "$version" > /package/DEBIAN/control
  printf '%s' "$version" > /package/usr/share/shipyard-review/version
  dpkg-deb --build /package /test-repo/shipyard-review-fixture_$version.deb
done
dpkg -i /test-repo/shipyard-review-fixture_1.0.deb
cd /test-repo
...
apt-get update -qq`]);
    ...
    fs.copyFileSync(path.resolve(__dirname,'../../playbooks/update.yml'),path.join(process.env.SHIPYARD_PLAYBOOKS_DIR,'update.yml'));
    ...
    const version=async()=> (await podman(['exec',name,'cat','/usr/share/shipyard-review/version'])).stdout;
