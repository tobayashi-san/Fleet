...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'fleet-recovery-key-')),file=path.join(root,'test.db');
...
test('key verification distinguishes absent evidence, matching key and damaged or mixed-key ciphertext',()=>{
 ...
 const previous=process.env.FLEET_KEY_SECRET;process.env.FLEET_KEY_SECRET='test-key';
 try{
  ...
  process.env.FLEET_KEY_SECRET='another-key';database.prepare('INSERT INTO app_settings VALUES (?)').run(encrypt('other-secret'));
