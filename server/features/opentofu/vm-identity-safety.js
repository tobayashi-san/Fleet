...
const { getProxmoxStateResources, isVmOwnershipMarker } = require('./proxmox-blueprints');
...
async function verifyVmIdentity({ workspace, vms, state, plan, checkConfiguration = true, request = requestProxmoxApi }) {
  ...
  for (const vm of vms) {
    ...
    if (change && !change.change?.actions?.every(action => ['no-op', 'read'].includes(action))) {
      if (Number(change.change.after?.vm_id) !== Number(vm.vm_id) || change.change.after?.node_name !== vm.node_name || !isVmOwnershipMarker(change.change.after?.description, vm.id)) throw new Error('The plan VM identity or ownership marker differs from this deployment. Create a new plan.');
    }
    const creating = change?.change?.actions?.includes('create');
    if (change?.change?.before && (Number(change.change.before.vm_id) !== Number(vm.vm_id) || change.change.before.node_name !== vm.node_name || !isVmOwnershipMarker(change.change.before.description, vm.id))) throw new Error('The saved plan references a different existing VM. Create a new plan.');
    ...
    if (!isVmOwnershipMarker(config?.description, vm.id) || !isVmOwnershipMarker(resource.values?.description, vm.id)) {
