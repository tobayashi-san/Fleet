  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-xl overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>VM definitions use a central platform connection. Manage credentials in Platform connections.</DialogDescription>
      </DialogHeader>
      {configQuery.isLoading || sourcesQuery.isLoading ? <div className="space-y-3 py-4"><div className="h-10 animate-pulse rounded-md bg-muted" /><div className="h-10 animate-pulse rounded-md bg-muted" /></div> : configQuery.isError || sourcesQuery.isError ? (
        ...
        <div className="space-y-1.5"><Label htmlFor="deployment-source">Proxmox platform</Label><select id="deployment-source" value={sourceId} onChange={event => setSourceId(event.target.value)} className="h-8 w-full rounded-sm border bg-background px-2.5 text-[13px]" disabled={sourcesQuery.isLoading || sources.length === 0}><option value="">{sourcesQuery.isLoading ? 'Loading…' : 'Select platform…'}</option>{sources.map(source => <option key={source.id} value={source.id}>{source.name} · {source.endpoint}</option>)}</select><p className="text-xs text-muted-foreground">Changes to the token, TLS, and default SSH key are made centrally in Platform connections.</p></div>
        ...
        {sources.length === 0 && <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-sm text-muted-foreground">There is no platform connection in this environment yet. Open Platform connections on the VM definitions page to add one.</div>}
