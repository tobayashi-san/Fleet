...
async function main() {
  ...
  const hosts = await request(app).get('/api/servers').set('Authorization', `Bearer ${token}`).set('X-Shipyard-Environment', 'recovery-env');
  const playbook = await request(app).get('/api/playbooks/recovered.yml').set('Authorization', `Bearer ${token}`).set('X-Shipyard-Environment', 'recovery-env');
