...
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'fleet-inventory-'));
...
test('creation uses a consistent explicit environment and rejects conflicting context', async () => {
 ...
 const created=await request(scopedApp).post('/servers').set('X-Fleet-Environment','staging').send(data);
 ...
 const mismatch=await request(scopedApp).post('/servers').set('X-Fleet-Environment','default').send({...data,name:'must-not-exist'});
