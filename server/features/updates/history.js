...
module.exports = function updateHistory(req, res) {
  ...
  const hostRows = db.db.prepare(`SELECT id, server_id, server_name_snapshot, action, status, started_at, completed_at, triggered_by
    FROM update_history WHERE environment_id = ? AND action IN ('system_update', 'reboot') ORDER BY started_at DESC LIMIT ?`).all(environmentId, LIMIT * 2)
    .filter(row => visible.has(row.server_id))
    .map(row => ({
      id: `host-${row.id}`,
      kind: row.action === 'reboot' ? 'reboot' : 'update',
      name: row.action === 'reboot' ? 'Reboot' : 'System update',
