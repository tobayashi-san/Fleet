    ...
    <>
      ...
      <Dialog open={!!upload} onOpenChange={open => { if (!open) cancelUpload(); }}>
        <DialogContent>
          ...
          {upload && <div className="space-y-4">
            ...
            {uploadMutation.isPending && <div className="space-y-1"><div role="progressbar" aria-label="Upload from browser to Shipyard" aria-valuemin={0} aria-valuemax={100} aria-valuenow={uploadProgress} className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary transition-[width]" style={{ width: `${uploadProgress}%` }} /></div><div className="text-right font-mono text-xs text-muted-foreground">{uploadProgress}% sent to Shipyard</div><p role="status" className="text-xs text-muted-foreground">{uploadProgress >= 100 ? 'Browser upload complete. Waiting for the destination host to confirm the write…' : 'Uploading from this browser. The destination write is not yet confirmed.'}</p></div>}
          </div>}
      ...
      <Dialog open={!!transfer} onOpenChange={open => { if (!open && !transferMutation.isPending) setTransfer(null); }}>
        <DialogContent>
          ...
          {transferMutation.isPending && <p role="status" className="text-sm text-muted-foreground">Transferring between hosts through Shipyard. Waiting for the destination write to finish; byte progress is not available for this transfer.</p>}
