...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'shipyard-import-review-'));process.env.DB_PATH=path.join(root,'db');process.env.SHIPYARD_SSH_DIR=path.join(root,'ssh');process.env.NODE_ENV='test';
...
test('preview exposes fingerprints without activation and import binds both reviewed identities',async()=>{
 const original=manager.getKeyInfo();const files=fs.readdirSync(process.env.SHIPYARD_SSH_DIR).sort();
 ...
 for(const override of [{expectedKeyId:'stale'},{expectedFingerprint:'SHA256:wrong'}]){
  const result=await request(app).post('/system/key/import').send({privateKey,...binding,...override});assert.equal(result.status,409);assert.equal(manager.getKeyInfo().id,original.id);assert.deepEqual(fs.readdirSync(process.env.SHIPYARD_SSH_DIR).sort(),files);
 }
