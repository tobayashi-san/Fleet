...
export function AgentOverview({proposedEnabled}:{proposedEnabled?:boolean}){
  ...
  return <div className="space-y-3 py-3 text-sm">
    ...
    {query.isError?<QueryErrorState compact title="Agent overview unavailable" error={query.error} onRetry={()=>void query.refetch()}/>:query.isPending?<p>Loading agent overview…</p>:<>
      {proposedEnabled !== undefined && <p role="status" className="rounded border p-3">After saving: {proposedEnabled ? `${query.data.counts.push} push and ${query.data.counts.pull} pull configurations become eligible; ${query.data.counts.ssh} hosts keep SSH.` : `${query.data.total} hosts use SSH selection; ${query.data.counts.push + query.data.counts.pull} configured agent hosts are affected.`} Agent health and fallback still determine actual collection.</p>}
