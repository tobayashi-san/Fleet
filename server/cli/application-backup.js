...
async function main(){
 ...
 const passphrase=process.env.SHIPYARD_BACKUP_PASSPHRASE;delete process.env.SHIPYARD_BACKUP_PASSPHRASE;
 let result;
 if(action==='verify-key')result=await verifyApplicationDatabaseKey(filename,passphrase,process.env.SHIPYARD_KEY_SECRET);
